...
The three baselines are compared below on 50 randomly sampled training windows — drag the slider (or type a number) to browse them. The orange curve is the linear extrapolation using only the end points of the input window, the purple is a linear fit to the CNA playercounts of the input window and the green is a linear fit to the log of the CNA. The blue curve is the input window the models see; the actual CNA future values, which they do not, are shown in grey in the background.
