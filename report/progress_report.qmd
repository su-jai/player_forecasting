...
![Various baseline model predictions on an example training window. The orange curve is a linear extrapolation using only the end points of the input window, the purple is a linear fit to the CNA playercounts of the input window and the green is a linear fit to the log of the CNA. The actual CNA future values are shown in the background.](figs/extrapolation_baselines.svg){#fig-baselines width=100%}
