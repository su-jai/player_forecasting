...
2. $\lambda \approx$ $(-1)\times$ slope of the linear regression of $\log\left(\text{CNA}\right)$ since the previous peak.


| Features | CV RMSE (log target) |
|---|---|
| Window + time + log peak (no decay features) | 0.4470 |
| Including decay rate $\lambda$ from method 1. | 0.4469 |
| Including decay rate $\lambda$ from method 2. | 0.4455 |
...
it can fail to converge. There may also not be sufficiently many datapoints from which to estimate the two parameters. In either of these cases we simply choose to reject the sample, which for our training dataset occurs 1.7% of the time. The model trained on these new features performs

| Features | CV RMSE (log target) |
|---|---|
| Including | 0.4429 |

It is clear that these decay rate features do not add significant information for the model to utilise.

# Aside: data extraction functions
One thing I have noticed whilst experimenting with these models is that I find myself constantly copy and pasting the bulk of the code for the "extractor" functions (whose job it is to scan across the dataset and output datapoints which have a specific collection of features) for each new / different choice of features that I am testing on. There must be a better way to perform this sort of experimentation.

A better way would be to separate these functions into their base components and combine them (i.e. modularise as much as possible). Immediately what comes to mind is to separate extractor functions which look only at the "local" input window of 56 days from the "global" functions which are extracting data which depends on the entire *past* temporal data at the $t=0$ point.

For example rather than have a separate extractor function for estimating the decay rate using a linear fit to log CNA playercounts and estimating the decay rate using a two-point estimation as done above, we could just have one local extractor function (as these features only depend on the 56 day input window) which can take as an input whatever function is to be applied to the input window directly. 



## Summary statistic features
As one final attempt to formulate "orthogonal" features we take each 56 day CNA input window and distill it into a handful of summary statistics that aim to capture uncorrelated features of the input data for the model to learn from.
